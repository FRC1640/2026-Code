...
public class CreateSysIdCommand {
  ...
  public static Command createCommand(Function<SysIdRoutine.Direction, Command> quasistaticCommand,
    ...
    Command sysIdCommand = new SequentialCommandGroup(
        quasistaticCommand
            .apply(SysIdRoutine.Direction.kForward).finallyDo(() -> stopMotors.run()).until(cancel),
        new WaitUntilCommand(startNext),
        quasistaticCommand.apply(SysIdRoutine.Direction.kReverse).finallyDo(() -> stopMotors.run())
            .until(cancel),
        new WaitUntilCommand(startNext),
        dynamicCommand.apply(SysIdRoutine.Direction.kForward).finallyDo(() -> stopMotors.run()).until(cancel),
        new WaitUntilCommand(startNext),
        dynamicCommand.apply(SysIdRoutine.Direction.kReverse).finallyDo(() -> stopMotors.run()).until(cancel));
