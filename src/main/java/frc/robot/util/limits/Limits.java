...
public class Limits {
  ...
  /**
   * Constructs a new {@code Limits} using the given range
   * and positive direction.
   * 
   * @param limit1 First limit.
   * @param limit2 Second limit.
   * @param positiveDir Boolean indicating the direction a
   * system will move when a positive output is applied (for
   * clamping utilities). A value of {@code true} indicates
   * that positive output moves the system closer to the high
   * limit, while a {@code false} value means the system would
   * approach the low limit. (The high limit is simply the
   * greater of the two.)
   */
