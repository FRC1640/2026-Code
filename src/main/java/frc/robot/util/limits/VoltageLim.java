...
public class VoltageLim {
   ...
   * @param voltage
   *            the base voltage to clamp.
   * @param bounds
   *            the limits for the motor
   * @return clamped voltage.
   */
  // public static double applyLimits(
  // Boolean highLimit, Boolean lowLimit, double pos, double voltage, Bounds
  // bounds) {
  ...
  public static double applyLimits(double pos, double voltage, boolean lowLimit, boolean highLimit) {
    double voltageClamped = voltage;
    if (!(Double.isNaN(voltageClamped) || Double.isNaN(pos))) {
