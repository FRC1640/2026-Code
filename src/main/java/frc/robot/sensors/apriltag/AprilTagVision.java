...
public class AprilTagVision extends PeriodicBase {
  ...
  public static class TrackingObservation {
    public int stale;
    public Translation3d vector;
    public Translation3d lastVector;

    public TrackingObservation(Translation3d vector) {
      stale = 0;
      this.vector = vector;
      lastVector = vector;
    }
  ...
  private final int staleThreshold = 4;
  ...
  public AprilTagVision(AprilTagVisionIO io, CameraConstant cameraConstants) {
    ...
    AlertsManager.addAlert(() -> !inputs.connected, "April tag vision disconnected.", AlertType.kError);
  }
  ...
  public Optional<PoseObservation> getTrigResult(Rotation2d gyroRotation) {
    ...
    PoseObservation observation = new PoseObservation(trigPoses.get(trigPoses.size() - 1).timestamp(),
        new Pose3d(averagePose), 0, trigPoses.size(), averageDistance, minimumDistance);
    // Logger.recordOutput(cameraName, null);
    // return Optional.of(bestPose);
    return Optional.of(observation);
  }

  private PoseObservation calculateTrigResult(TrigTargetObservation observation, Translation3d cameraToTag,
    ...
    Translation3d cameraToTagFieldFrame = cameraToTag.rotateBy(cameraRotation)
        .rotateBy(new Rotation3d(0, 0, gyroRotation.getRadians()));

    // Calculate camera position in field coordinates
    Translation3d fieldToCamera = FieldConstants.aprilTagLayout.getTagPose(observation.fiducialId()).get()
        .getTranslation().minus(cameraToTagFieldFrame);

    // Convert camera displacement to field coordinates
    Translation3d cameraDisplacementField = cameraToRobot.rotateBy(new Rotation3d(0, 0, gyroRotation.getRadians()));
    ...
    Pose3d robotPose = new Pose3d(fieldToRobotTranslation, new Rotation3d(0, 0, gyroRotation.getRadians()));

    return new PoseObservation(observation.timestamp(), robotPose, 0, 1,
  ...
  public void setLocalAlignId(int id) {
    localAlignId = id;
  }
  ...
  private Optional<Translation2d> getLocalAlignVector(int id) {
    ...
    if (observation.isPresent()) {
      ...
      Translation2d frontToTag = inputs.cameraDisplacement.getTranslation().plus(vector).toTranslation2d()
          .minus(new Translation2d(RobotConstants.RobotDimensions.robotLengthLocalAlign / 2, 0));
      localAlignOutput = Optional.of(frontToTag);
      lastLocalVector = localAlignOutput;
      staleCount = 0;
      return localAlignOutput;
    } else if (lastLocalVector.isPresent()) {
      localAlignOutput = Optional.empty();
      staleCount++;
  ...
  public void updateTagTracking() {
    ...
    for (int id : trackingMap.keySet()) {
      if (!idsObserved.contains(id)) {
        ...
        if (trackingObservation.stale >= staleThreshold) {
          trackingObservation.lastVector = null;
        }
      }
    }
  }
  ...
  private Translation3d calculateCameraVector(TrigTargetObservation observation) {
    ...
    double deltaZ = FieldConstants.aprilTagLayout.getTagPose(observation.fiducialId()).get().getZ()
        - cameraToRobot.getZ();
    ...
    Translation3d cameraToTagCameraFrame = new Translation3d(distance2d * Math.cos(tx), distance2d * Math.sin(tx),
        -deltaZ);
  ...
  public double getPhotonDistFactor(PoseObservation observation) {
    double distFactor = Math.pow(observation.minimumTagDistance(), 2.0) / observation.tagCount()
  ...
  public double getTrigDistFactor(PoseObservation observation) {
    double distFactor = Math.pow(observation.averageTagDistance(), 2) / observation.tagCount()
        * getStandardDeviation();
  ...
  public void periodic() {
    ...
    updateTagTracking();
    localVector = getLocalAlignVector(localAlignId);
    Logger.recordOutput("AprilTagVisionLocal/" + displayName + "/IsPresent", localVector.isPresent());

    Logger.recordOutput("idtouse", localAlignId);

    if (lastLocalVector.isPresent()) {
      Logger.recordOutput("AprilTagVisionLocal/" + displayName + "/LocalAlignVectorLast", lastLocalVector.get());
    } else {
      Logger.recordOutput("AprilTagVisionLocal/" + displayName + "/LocalAlignVectorLast", new Translation2d());
    }
    if (localVector.isPresent()) {
      Logger.recordOutput("AprilTagVisionLocal/" + displayName + "/LocalAlignVector", localVector.get());
    } else {
      Logger.recordOutput("AprilTagVisionLocal/" + displayName + "/LocalAlignVector", new Translation2d());
    }
