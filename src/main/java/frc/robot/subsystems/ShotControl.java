...
public class ShotControl {
    ...
    private TurretSetpoint getScoringSetpoint() {
        ...
        Translation2d targetOffset = getNearestShootingPoint(robotPose.get()).getTranslation().minus(turretPose.getTranslation()); // fieldcentric
        ...
        TurretSetpoint output = new TurretSetpoint(turretAngle, desiredTurretVelocity,
                Units.radiansToDegrees(Math.acos(planarProjectileVelocity.getNorm() / shooterVelocity)), shooterVelocity);
    ...
    private TurretSetpoint getFerryingSetpoint() {
        ...
        double shooterVelocity = Math.sqrt((FieldConstants.gravityEarth * targetOffset.getNorm()) / Math.sin(Math.PI / 4));
        Rotation2d hoodAngle = new Rotation2d(Math.PI / 4);
        double desiredTurretVelocity = lastSetpoint != null ? (hoodAngle.getRadians() - lastSetpoint.turretAngleRad()) / 0.02 : 0;

        TurretSetpoint output = new TurretSetpoint(targetOffset.getAngle().minus(robotPose.get().getRotation()).getRadians(), desiredTurretVelocity,
